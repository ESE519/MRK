...
typedef struct nrk_driver
{
	...
	uint8_t (*devicemanager)(uint8_t state,uint8_t opt,uint8_t*buf,uint8_t size); 
	// function pointer to the driver call

} NRKDriver;


int8_t nrk_register_driver(uint8_t (*devicemanager)(uint8_t, uint8_t, uint8_t*, uint8_t),uint8_t driver_name);
