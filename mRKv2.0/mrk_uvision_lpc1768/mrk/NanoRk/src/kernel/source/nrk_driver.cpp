...
int8_t nrk_register_driver(uint8_t (*devicemanager)(uint8_t, uint8_t, uint8_t*, uint8_t),uint8_t dev_index)
{
    if(_nrk_driver_count<NRK_MAX_DRIVER_CNT)
    {
        nrk_drivers[_nrk_driver_count].dev_id=dev_index;
        nrk_drivers[_nrk_driver_count].devicemanager=devicemanager;
        _nrk_driver_count++;
        return NRK_OK;
    }
    else
